...
const startServer = async () => {
  try {
    ...
    // In development mode, we can bypass the database connection check
    if (
      process.env.NODE_ENV === "development" &&
      process.env.BYPASS_DB_CHECK === "true"
    ) {
      logger.warn("Bypassing database connection check in development mode");
      dbConnected = true;
    } else {
      ...
      if (!dbConnected) {
        if (process.env.NODE_ENV === "development") {
          logger.warn(
            "Database connection check failed, but continuing in development mode"
          );
        } else {
          throw new Error(
            "Database connection check failed after maximum retries"
          );
        }
      }
    }
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        logger.warn(
          "Failed to set up model associations, but continuing in development mode",
          error
        );
      } else {
        throw error;
    ...
    try {
      // In development mode with BYPASS_DB_CHECK, skip database operations
      if (
        process.env.NODE_ENV === "development" &&
        process.env.BYPASS_DB_CHECK === "true"
      ) {
        logger.warn("Bypassing database sync and seeding in development mode");
      } else if (process.env.NODE_ENV === "development" && !dbConnected) {
        // In development mode, if database connection failed, we can still proceed
        // but we should warn the user
        logger.warn(
          "Database connection failed, but continuing in development mode without sync and seeding."
        );
        logger.warn(
          "Please make sure PostgreSQL is running on localhost:5432 and the database exists."
        );
        logger.warn(
          "You can also set BYPASS_DB_CHECK=true in .env to bypass database checks."
        );
      } else {
